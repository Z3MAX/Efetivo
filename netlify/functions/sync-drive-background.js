/**
 * sync-drive-background.js — Netlify Background Function
 * Executa async (até 15 min). Netlify retorna 202 imediatamente ao cliente.
 * Resultado salvo em efetivo_sync_status no banco.
 * POST /api/sync-drive
 */
...
async function salvarStatus(db, status, dados) {
  await db`
    INSERT INTO efetivo_sync_status (id, status, detalhe, iniciado_at, finalizado_at)
    VALUES (1, ${status}, ${JSON.stringify(dados)}::jsonb, COALESCE((SELECT iniciado_at FROM efetivo_sync_status WHERE id=1), NOW()), ${status !== 'rodando' ? 'NOW()' : null}::timestamptz)
    ON CONFLICT (id) DO UPDATE SET
      status = EXCLUDED.status,
      detalhe = EXCLUDED.detalhe,
      iniciado_at = CASE WHEN efetivo_sync_status.status = 'rodando' THEN efetivo_sync_status.iniciado_at ELSE NOW() END,
...
exports.handler = async (event) => {
  ...
  // Garante que a tabela de status existe
  await db`
    CREATE TABLE IF NOT EXISTS efetivo_sync_status (
      id INT PRIMARY KEY,
  ...
  try {
    ...
    if (!files.length) {
      await db`UPDATE efetivo_sync_status SET status='erro', detalhe='{"erro":"Nenhum arquivo .xlsx encontrado"}'::jsonb, finalizado_at=NOW() WHERE id=1`
      return
    }
    ...
    for (const r of rows) {
      ...
      presRows.push({ matricula: mat, data, proj: mapProjeto(r['Cod Centro de Custo'], r['Desc Centro de Custo']), fonte: 'ponto' })
    }
    ...
    console.error('[sync-drive-background]', e)
    await db`
      UPDATE efetivo_sync_status SET
        status = 'erro',
