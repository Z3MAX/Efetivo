"""
init_db.py — Cria o banco SQLite local e popula projetos/relógios.
...
def init():
    ...
    conn.executemany(
        "INSERT OR IGNORE INTO efetivo_projetos (codigo, nome) VALUES (?,?)", PROJETOS)
    conn.executemany(
        "INSERT OR IGNORE INTO efetivo_relogios (relogio, descricao, codigo_projeto) VALUES (?,?,?)", RELOGIOS)
    conn.executemany(
        "INSERT OR IGNORE INTO efetivo_usuarios (email, nome, senha, perfil) VALUES (?,?,?,?)", USUARIOS)
    conn.commit()
    conn.close()
    print(f"OK Banco criado: {DB}")
    print(f"  {len(PROJETOS)} projetos | {len(RELOGIOS)} relogios | {len(USUARIOS)} usuario(s)")
